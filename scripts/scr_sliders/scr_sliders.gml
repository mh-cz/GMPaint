...
function h_slider(x1, y1, w, h, val, s_id, mx = device_mouse_x_to_gui(0), my = device_mouse_y_to_gui(0)) {
	...
	if _selected_slider == s_id {
			newval = clamp((mx - x1) / abs(x1 - x2), 0, 1);
	}
	else if mouse_check_button_pressed(mb_left) 
	and point_in_rectangle(mx, my, x1, y1, x2, y2) {
		_selected_slider = s_id;
	}
